...
public class L3Cache implements Cache<String, Event> {

    private final static int MAX_SIZE = 50;
    private final CacheType cacheType = CacheType.L3;
    ...
    public synchronized void insertEvent(String key, Event event) {
        ...
        if (internalQueue.size() < MAX_SIZE) {
            ...
            if (fetchedEvent == null) {
                ...
                System.out.println("[L3Cache] Size is almost full, emptying older record : " + olderEvent.getKey());
                internalQueue.add(event);
            }
        }
    ...
    public Event fetchEvent(String key) {
        ...
        if (fetchedEvent == null) {
            System.out.println("[L3Cache] Event doesn't exist for key: " + key + " searching in L4Cache.");
            fetchedEvent = getPredecessorCache().fetchEvent(key);

            if (fetchedEvent == null) {
                System.out.println("[L3Cache] Event doesn't exist in predecessor : " + key);
            } else {
