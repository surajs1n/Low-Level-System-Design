...
public class L1Cache implements Cache<String, Event> {

    private final static int MAX_SIZE = 2;
    private final CacheType cacheType = CacheType.L1;
    ...
    public synchronized void insertEvent(String key, Event event) {
        ...
        if (internalQueue.size() < MAX_SIZE) {
            ...
            if (fetchedEvent == null) {
                ...
                System.out.println("[L1Cache] Size is almost full, emptying older record : " + olderEvent.getKey());
                internalQueue.add(event);
            }
        }
    ...
    public Event fetchEvent(String key) {
        ...
        if (fetchedEvent == null) {
            System.out.println("[L1Cache] Event doesn't exist for key: " + key + " searching in L2Cache.");
            fetchedEvent = getPredecessorCache().fetchEvent(key);

            if (fetchedEvent == null) {
                System.out.println("[L1Cache] Event doesn't exist in predecessor : " + key);
            } else {
